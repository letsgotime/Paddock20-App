 ...
 */
export async function getEnhancedF1Weather(req: Request, res: Response) {
  try {
    ...
    console.log('Enhanced F1-Style Weather API called with params:', req.query);
    ...
    if (
      ...
      console.log('Returning cached enhanced F1 weather data');
    ...
    const f1Data = calculateF1Metrics(
      current, 
      oneCallData, 
      forecastData, 
      airQualityData, 
    ...
    console.error('Error fetching enhanced F1 weather data:', error.message);
    res.status(500).json({ 
      error: 'Failed to fetch enhanced F1 weather data',
...
function calculateF1Metrics(
  ...
  return {
    ...
    tireStrategy: {
      optimal_compound: (tireRecommendations as any).optimal_compound,
      tire_temperature: {
        surface: (tireRecommendations as any).surface_temp,
        core: (tireRecommendations as any).core_temp,
        optimal_window: (tireRecommendations as any).optimal_window,
        warmup_time: (tireRecommendations as any).warmup_time
      },
      pressure: {
        recommendation: (tireRecommendations as any).pressure_recommendation,
        front_pressure_delta: (tireRecommendations as any).front_pressure_delta,
        rear_pressure_delta: (tireRecommendations as any).rear_pressure_delta,
        pressure_buildup_rate: (tireRecommendations as any).pressure_buildup_rate
      },
      wear: {
        expected_wear_rate: (tireRecommendations as any).wear_rate,
        wear_pattern: (tireRecommendations as any).wear_pattern,
        graining_risk: (tireRecommendations as any).graining_risk,
        blistering_risk: (tireRecommendations as any).blistering_risk,
        management_strategy: (tireRecommendations as any).management_strategy
      }
    },
    ...
    sunData: {
      ...
      daylight_remaining: isDaytime ? Math.round((sunset.getTime() - now.getTime()) / 60000) : 0,
      glare_risk: getGlareRisk(current, sunrise, sunset, now),
...
// Get track condition
function getTrackCondition(isRaining: boolean, isSnowing: boolean, trackTemp: number, gripIndex: number): string {
...
function getGripAssessment(gripIndex: number): string {
...
function getPrecipitationIntensity(currentData: any): string {
...
function calculateTemperatureTrend(hourlyData: any[]): { trend: number; description: string } {
...
function identifyConditionChanges(hourlyData: any[]): string {
...
function analyzeRainForecast(hourlyData: any[], dailyData: any[]): any {
  ...
  if (!rainExpected && dailyData.length > 0) {
    for (let i = 0; i < Math.min(2, dailyData.length); i++) {
      ...
      if (day.pop > 0.3) {
        rainExpected = true;
        // Use special number values for daily forecasts to differentiate from hourly
        timeToRain = i === 0 ? 100 : 200; // 100 = later today, 200 = tomorrow
        intensity = "Unknown";
        confidence = "Moderate";
        break;
      }
...
function predictTrackEvolution(current: any, hourlyData: any[], dailyData: any[], currentGrip: number): any {
...
function calculatePerformanceTireRecommendations(trackTemp: number, isRaining: boolean, gripIndex: number): any {
  // Base values
...
function calculateStreetTireRecommendations(trackTemp: number, isRaining: boolean, gripIndex: number): any {
  // Base values
...
function formulateTireManagementStrategy(
  compound: string, 
  wearRate: string, 
  grainingRisk: string, 
  blisteringRisk: string, 
  isRaining: boolean
): string {
  if (isRaining) {
...
// Get track alerts
function getTrackAlerts(current: any, gripIndex: number, isRaining: boolean): string[] {
  const alerts: string[] = [];
...
function calculateCrosswindEffect(windSpeed: number, windDirection: number): string {
...
function calculateBrakingEfficiency(gripIndex: number, trackTemp: number, isRaining: boolean): number {
  let base = gripIndex;
...
function calculateBrakingDistance(efficiency: number, units: string): any {
...
function getHumidityImpact(humidity: number): string {
...
function getAirQualityImpact(aqi: number): string {
  switch(aqi) {
...
function getCoolingEfficiency(temp: number, windSpeed: number): string {
...
function getEngineTempDelta(temp: number): string {
...
function getEngineMapping(current: any, airQualityIndex: number, humidity: number): string {
...
function getFuelConsumptionDelta(temp: number, humidity: number, airQualityIndex: number): string {
  let delta = 0;
...
function getHeatDissipation(temp: number, windSpeed: number): string {
...
function getBrakeTemperatureManagement(temp: number, isRaining: boolean): string {
...
function getDownforceEfficiency(temp: number, pressure: number): number {
...
function getDragCoefficientAdjustment(temp: number, pressure: number): string {
...
function getTorqueCurveAdjustment(humidity: number, temp: number, airQualityIndex: number): any {
...
function getGlareRisk(current: any, sunrise: Date, sunset: Date, now: Date): string {
  // No glare at night
  if (now < sunrise || now > sunset) return "None";
  
  // Higher risk around sunrise/sunset
  const sunriseTime = now.getTime() - sunrise.getTime(); // ms from sunrise
  const sunsetTime = sunset.getTime() - now.getTime(); // ms until sunset
...
// Get glare direction
function getGlareDirection(current: any, sunrise: Date, sunset: Date, now: Date): string {
  // No glare at night
  if (now < sunrise || now > sunset) return "None";
...
function adjustGripForSector(baseGrip: number, sector: number): number {
...
function generateDriverCoaching(current: any, gripIndex: number, isRaining: boolean, trackTemp: number): any {
...
function getStintLengthRecommendation(
  wearRate: string, 
...
function getCompoundProgression(
