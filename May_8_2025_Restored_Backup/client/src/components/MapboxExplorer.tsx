...
import { mapboxService } from '@/services/location/mapboxService';
import { LocationInfo } from '@/contexts/EnhancedLocationContext';
import { useToast } from '@/components/ui/use-toast';

const MapboxExplorer: React.FC = () => {
  ...
  const refreshLocation = async () => {
    ...
    try {
      ...
      const geocodeResult = await mapboxService.reverseGeocode(latitude, longitude);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setSearching(true);
    try {
      const result = await mapboxService.geocode(searchQuery);
  ...
  const clearCache = () => {
    ...
    toast({
      title: 'Cache Cleared',
      description: 'The Mapbox geocoding cache has been cleared',
    });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
        ...
        <Card className="w-full md:w-1/2">
          <CardHeader>
            ...
            <CardDescription>Your detected location with Mapbox geocoding</CardDescription>
          </CardHeader>
        ...
        <Card className="w-full md:w-1/2">
          <CardHeader>
            ...
            <CardDescription>
              Search for any location using Mapbox geocoding
            </CardDescription>
          </CardHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Mapbox Geocoding Cache</CardTitle>
          <CardDescription>
            7-day caching to optimize API usage
          </CardDescription>
        </CardHeader>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center">
              <div className="text-sm text-muted-foreground">
                <strong>Note:</strong> This implementation uses 7-day caching with fallback to cached data during API errors.
              </div>
