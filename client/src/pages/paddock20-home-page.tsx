...
const Paddock20HomePage: React.FC = () => {
  ...
  // Check if we need to show the beta welcome page - Run once at startup
  useEffect(() => {
    // If this is a production deployment and we haven't seen the welcome page, show it
    const isDeployment = window.location.hostname.includes('replit.app');
    
    if (isDeployment) {
      // Get either global flag or user-specific flag
      const globalOnboardingComplete = localStorage.getItem('paddock20_onboarding_completed') === 'true';
      
      // Get user profile to check for user-specific flags
      ...
      if (userProfileStr) {
        try {
          ...
          if (userProfile?.id) {
            userOnboardingComplete = localStorage.getItem(`paddock20_beta_onboarding_complete_${userProfile.id}`) === 'true';
          }
        } catch (e) {
          console.error('Error checking user profile for onboarding status:', e);
        }
      }
      
      // If neither global nor user-specific onboarding is complete, redirect to beta welcome
      if (!globalOnboardingComplete && !userOnboardingComplete) {
        console.log('Redirecting to beta welcome page - onboarding not completed');
      ...
      <div className="relative z-10 w-full max-w-[100%] sm:max-w-[1200px] mx-auto px-2 sm:px-4 pb-6">
        ...
        <div className="mb-4">
          ...
          <div className="text-center text-white mb-2 flex flex-col items-center">
            {/* Time and Date Displays in a row */}
            <div className="flex flex-wrap justify-center mb-3">
              <div className="time-display py-1 px-2 bg-[#0a0a0a] my-1 mx-1 inline-block border-l-2 border-blue-500 border-t border-b border-r border-gray-800 rounded-sm">
              ...
              <div className="date-display py-1 px-2 bg-[#0a0a0a] my-1 mx-1 inline-block border-l-2 border-blue-500 border-t border-b border-r border-gray-800 rounded-sm">
                <div className="flex items-center">
            ...
            <div className="flex flex-wrap justify-center space-x-2">
              {/* Time Format options */}
              <div className="time-format-dropdown cursor-pointer bg-black/70 border border-blue-900/20 rounded-md hover:border-blue-500/40 transition-all duration-300 hover:bg-black/90 hover:shadow-[0_0_10px_rgba(75,156,211,0.15)]"
                  onClick={() => setShowTimeOptions(!showTimeOptions)}>
                <div className="px-3 py-1 text-xs text-blue-400/80 flex items-center">
                  <div className="h-1.5 w-1.5 bg-blue-500 rounded-full animate-pulse mr-1.5"></div>
                  <span>Time Format: {timeFormat === '24h' ? '24-hour' : '12-hour'}</span>
                </div>
                
                {/* Time Format Options */}
                {showTimeOptions && (
                  <div className="absolute z-50 mt-1 bg-gradient-to-b from-[#111] to-black border border-blue-900/50 rounded-sm shadow-[0_4px_20px_rgba(0,0,0,0.5)] p-0 overflow-hidden w-48">
              ...
              {/* Date Format options */}
              <div className="date-format-dropdown cursor-pointer bg-black/70 border border-blue-900/20 rounded-md hover:border-blue-500/40 transition-all duration-300 hover:bg-black/90 hover:shadow-[0_0_10px_rgba(75,156,211,0.15)]"
                  onClick={() => setShowDateOptions(!showDateOptions)}>
                <div className="px-3 py-1 text-xs text-blue-400/80 flex items-center">
                  <div className="h-1.5 w-1.5 bg-blue-500 rounded-full animate-pulse mr-1.5"></div>
                  <span>Date Format: {dateFormat.toUpperCase()}</span>
                </div>
                
                {/* Date Format Options */}
                {showDateOptions && (
                  <div className="absolute z-50 mt-1 bg-gradient-to-b from-[#111] to-black border border-blue-900/50 rounded-sm shadow-[0_4px_20px_rgba(0,0,0,0.5)] p-0 overflow-hidden w-56">
                    <div className="bg-blue-900/20 px-2 py-1 text-[10px] uppercase text-blue-400/80 font-medium tracking-wider">
                      Select Format
                    </div>
                    <div 
          ...
          <div className="overflow-hidden rounded-lg border border-gray-800 bg-black/20 mb-4">
            ...
            <div className="p-3 border-t border-gray-800">
              ...
              <div className="grid grid-cols-3 gap-2 mt-2">
                <Link to="/garage-vault?section=maintenance&action=add-maintenance" className="text-xs bg-green-600/20 text-green-400 hover:bg-green-600/30 transition-colors px-2 py-1 rounded flex items-center justify-center">
                ...
                <Link to="/garage-vault?section=gloss&action=log-wash" className="text-xs bg-blue-600/20 text-blue-400 hover:bg-blue-600/30 transition-colors px-2 py-1 rounded flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 mr-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                <Link to="/garage-vault?section=modifications&action=add-mod" className="text-xs bg-purple-600/20 text-purple-400 hover:bg-purple-600/30 transition-colors px-2 py-1 rounded flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 mr-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
