      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-6">
          ...
          <div className="mt-4 bg-gradient-to-r from-gray-900 to-black p-4 rounded-lg border border-blue-700 shadow-lg">
            ...
            {/* Vehicle Selection - Moved inside F1-Grade Telemetry */}
            <div className="mb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div className="mt-6 pt-6 border-t border-blue-900/30">
                ...
                <div className="bg-black/30 rounded-lg p-3 border border-blue-900/30 mb-3">
                  <div className="flex flex-wrap gap-2 mb-3">
                    <button 
                      onClick={() => setSelectedHeatmapMetric('performance')}
                      className={`px-3 py-1 rounded-full text-xs font-medium ${selectedHeatmapMetric === 'performance' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}
                    >
                      Overall Performance
                    </button>
                    <button 
                      onClick={() => setSelectedHeatmapMetric('speed')}
                      className={`px-3 py-1 rounded-full text-xs font-medium ${selectedHeatmapMetric === 'speed' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}
                    >
                      Speed
                    </button>
                    <button 
                      onClick={() => setSelectedHeatmapMetric('acceleration')}
                      className={`px-3 py-1 rounded-full text-xs font-medium ${selectedHeatmapMetric === 'acceleration' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}
                    >
                      Acceleration
                    </button>
                    <button 
                      onClick={() => setSelectedHeatmapMetric('cornering')}
                      className={`px-3 py-1 rounded-full text-xs font-medium ${selectedHeatmapMetric === 'cornering' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}
                    >
                      Cornering G-Force
                    </button>
                    <button 
                      onClick={() => setSelectedHeatmapMetric('elevation')}
                      className={`px-3 py-1 rounded-full text-xs font-medium ${selectedHeatmapMetric === 'elevation' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}
                    >
                      Elevation Changes
                    </button>
                    <button 
                      onClick={() => setSelectedHeatmapMetric('temperature')}
                      className={`px-3 py-1 rounded-full text-xs font-medium ${selectedHeatmapMetric === 'temperature' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}
                    >
                      Surface Temperature
                    </button>
                  </div>
                  ...
                  <div className="flex justify-between items-center mb-2">
                    <div>
                      <span className="text-gray-400 text-xs">Heatmap Mode: </span>
                      <select 
                        value={heatmapMode}
                        onChange={(e) => setHeatmapMode(e.target.value as 'real-time' | 'simulated')}
                        className="bg-gray-800 text-white text-xs rounded border border-gray-700 px-2 py-1"
                      >
                        <option value="real-time">Real-Time</option>
                        <option value="simulated">Pre-Drive Simulation</option>
                      </select>
                    </div>
                    
                    <div>
                      {heatmapMode === 'real-time' ? (
                        <div className="flex gap-2">
                          {isCollectingData ? (
                            <button 
                              onClick={stopRealTimeCollection}
                              className="bg-red-600 hover:bg-red-700 text-white text-xs font-medium px-3 py-1 rounded flex items-center"
                            >
                            ...
                            <button 
                              ...
                              className="bg-green-600 hover:bg-green-700 text-white text-xs font-medium px-3 py-1 rounded flex items-center"
                            >
                              <span className="h-2 w-2 bg-white rounded-full mr-1.5"></span>
                              Start GPS
                            </button>
                          )}
                        </div>
                        ...
                        <button 
                          onClick={() => generateRouteHeatmapData(routeWaypoints)}
                  ...
                  <div className="relative bg-black rounded-lg overflow-hidden h-[400px] border border-blue-900/30">
                    {heatmapData.length > 0 ? (
                      <RoutePerformanceHeatmap 
                        points={heatmapData}
                        width={800}
                        height={400}
                        colorScale={selectedHeatmapMetric === 'temperature' ? 'temperature' : 'performance'}
                        showLegend={true}
                        realTimeMode={heatmapMode === 'real-time'}
                        onHeatmapClick={(point) => console.log('Clicked point:', point)}
                      />
                    ) : (
                      <div className="absolute inset-0 flex items-center justify-center text-gray-500 text-sm">
                        {heatmapMode === 'real-time' ? 
                          "Real-time data will appear when GPS tracking begins" : 
                          "Click 'Generate Simulation' to view route performance heatmap"}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-2 text-xs text-gray-400 flex items-center">
                    ...
                    <span>
                      {heatmapMode === 'real-time' ? 
                        "Hover over any point to view detailed performance metrics. Real-time data uses device sensors." : 
                        "Simulated data uses route profile and vehicle characteristics to estimate performance."}
                    </span>
                  </div>
                </div>
              </div>
