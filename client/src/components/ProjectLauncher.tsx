...
interface ProjectLauncherProps {
  ...
  onClose: () => void;
  onProjectSubmit?: (project: ProjectEntry) => void;
}

const ProjectLauncher: React.FC<ProjectLauncherProps> = ({ 
  vehicles, 
  selectedVehicleId,
  onClose,
  onProjectSubmit
  ...
  const submitProject = () => {
    ...
    if (!entry.vehicleId) {
      toast({
        title: "Missing Information",
        description: "Please select a vehicle for this project",
        variant: "destructive"
      });
      return;
    }
  ...
  const selectedVehicle = vehicles.find(v => v.id === entry.vehicleId) || null;

  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="relative bg-zinc-900 border border-zinc-800 rounded-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto">
        <button 
          onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-zinc-800 rounded-lg p-4">
            ...
            {selectedVehicle && (
              <div className="mt-3 p-3 bg-zinc-900 rounded-lg border border-zinc-800 flex items-center">
                <div className="w-16 h-16 bg-zinc-800 rounded-lg overflow-hidden mr-3 flex-shrink-0">
                  {selectedVehicle.image_url ? (
                    ...
                    <Car className="w-full h-full p-3 text-gray-500" />
                  )}
                </div>
                <div>
                  ...
                  <p className="text-sm text-gray-400">
                    {selectedVehicle.trim} • {selectedVehicle.color} • {selectedVehicle.license_plate || "No plate"}
                  </p>
                </div>
              </div>
            )}
