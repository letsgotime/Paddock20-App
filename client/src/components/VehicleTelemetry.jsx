  ...
  return (
    <div className="bg-black text-white">
      <div className="max-w-7xl mx-auto px-4 mb-8">
          ...
          {expanded.specs && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-10 gap-y-4 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">{vehicle.torque || "N/A"} lb-ft</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">{vehicle.fuelEconomy || "N/A"} mpg</span>
              </div>
              {vehicle.aerodynamics && (
                <>
                  <div className="flex justify-between">
                    <span className="text-gray-400">Drag Coefficient:</span>
                    <span className="text-white">{vehicle.aerodynamics.dragCoefficient || "N/A"}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">Downforce:</span>
                    <span className="text-white">{vehicle.aerodynamics.downforce || "N/A"}</span>
                  </div>
                  <div className="flex justify-between">
        ...
        {/* Service History */}
        <div className="bg-gradient-to-br from-gray-900 to-black p-6 rounded-xl border border-gray-800 mb-6">
          <div className="flex items-center justify-between mb-4" onClick={() => toggleSection('history')}>
            <h4 className="text-lg font-semibold text-white flex items-center cursor-pointer">
              <Wrench className="mr-2 h-5 w-5 text-amber-500" />
              Service History
            </h4>
            <button className="text-gray-400 hover:text-white">
              {expanded.history ? <ChevronDown className="h-5 w-5" /> : <ChevronRight className="h-5 w-5" />}
            </button>
          </div>
          
          {expanded.history && vehicle.maintenanceRecords && (
            <div className="space-y-4">
              {vehicle.maintenanceRecords.map((record, index) => (
                <div key={index} className="bg-gray-900/40 p-4 rounded-lg border border-gray-800">
                  <div className="flex justify-between mb-2">
                    <div className="font-medium text-white">{record.service}</div>
                    <div className="text-gray-400 text-sm">{record.date}</div>
                  </div>
                  <div className="flex justify-between text-sm">
                    <div className="text-gray-400">Mileage: <span className="text-white">{record.mileage.toLocaleString()}</span></div>
                    <div className="text-gray-400">Cost: <span className="text-green-400">${record.cost.toLocaleString()}</span></div>
                  </div>
                </div>
              ))}
