...
const ContextualWeatherRoutePlanner: React.FC<RoutePlannerProps> = ({
  ...
  const [journeyDuration, setJourneyDuration] = useState<number>(30); // in minutes
  const [journeyWeatherConditions, setJourneyWeatherConditions] = useState<any[]>([]);
  const [showPlannerResults, setShowPlannerResults] = useState<boolean>(false);
  const [journeyRiskLevel, setJourneyRiskLevel] = useState<'low' | 'moderate' | 'high' | 'severe'>('low');
  ...
  // Generate journey forecast points
  const generateJourneyWeatherPoints = (startTime: Date, endTime: Date) => {
    if (!forecastData || !forecastData.list) return [];
    
    const journeyPoints = [];
    const totalDuration = endTime.getTime() - startTime.getTime();
    const numPoints = Math.min(5, Math.max(2, Math.floor(journeyDuration / 30) + 1));
        ...
        {!showPlannerResults ? (
          ...
          <div className="animate-fadein">
            ...
            <div className="mt-2 p-3 rounded-lg bg-blue-900/10 border border-blue-900/30">
              <h5 className="text-blue-400 text-sm font-semibold mb-2">Journey Tips:</h5>
